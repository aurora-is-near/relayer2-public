...
type processIndexerState func(*IndexerRefiner) processIndexerState
...
// NewIndexerRefiner creates the indexer, the db.Handler should not be nil
// Initialize indexer with broker only if the JSON-RPC server supports subscription,
// otherwise broker must be nil
func NewIndexerRefiner(
	config *Config,
	dbh db.Handler,
	b broker.Broker,
) (*IndexerRefiner, error) {
	logger := log.Log()

	batchSize := uint64(config.SubFolderBatchSize)
	subBlock := config.FromBlock / batchSize * batchSize
	subBlockPath := filepath.Join(config.SourceFolder, fmt.Sprintf("%v", subBlock))
	filePath := filepath.Join(subBlockPath, fmt.Sprintf("%v.json", config.FromBlock))

	return &IndexerRefiner{
		dbh: dbh,
		b:   b,
		l:   logger,
		c:   config,
...
// TODO: use ctx for requests fot gracefully shutdown
func (i *IndexerRefiner) Start(ctx context.Context) {
	i.lock.Lock()
	defer i.lock.Unlock()
...
func (i *IndexerRefiner) evalError(err error, onRetry processIndexerState, onFail processIndexerState) processIndexerState {
	if err == nil {
		return nil
	}
...
func read(i *IndexerRefiner) processIndexerState {
...
//	On failure, retries for Config.RetryCountOnFailure and if all retries fails then stops indexer
func insert(i *IndexerRefiner) processIndexerState {
...
func removeFile(i *IndexerRefiner) processIndexerState {
...
// block in any case.
func removeFolder(i *IndexerRefiner) processIndexerState {
...
func increment(i *IndexerRefiner) processIndexerState {
...
func stop(i *IndexerRefiner) processIndexerState {
...
func publish(i *IndexerRefiner) processIndexerState {
	if i.b != nil {
