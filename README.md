...
| indexer.source                                | INDEXER_SOURCE                                          | `refiner`                                    | ❌              | the source of blocks for the indexer. Can be one of `refiner, blocksApi`. `refiner` reads from JSON files generated by the refined application. `blocksApi` reads from the gRPC blocks endopint.                                                                          |
...
| indexer.blocksApiUrl                          | INDEXER_BLOCKSAPIURL                                    |                                              | ❌              | when `indexer.source` is set to `blocksApi`, this sets the base URL for the gRPC to connect to             |
| indexer.blocksApiStream                       | INDEXER_BLOCKSAPISTREAM                                 |                                              | ❌              | the name of the stream to read from `indexer.blocksApiUrl`             |
| indexer.blocksApiToken                        | INDEXER_BLOCKSAPITOKEN                                  |                                              | ❌              | the secret token used to authenticate with `indexer.blocksApiUrl`             |
